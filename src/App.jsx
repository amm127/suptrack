    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    <div id="suptrack-content" style={{flex:1,padding:"38px 44px",maxWidth:980,overflowY:"auto"}}>
      ...
      {addInternOpen&&<AddInternModal T={t} groups={groups} lists={lists}
        onSave={async newIntern=>{const row={supervisor_id:session.user.id,name:newIntern.name,preferred_name:newIntern.preferredName||"",initials:newIntern.initials||"",pronouns:newIntern.pronouns||"",gender:newIntern.gender||"",race:newIntern.race||"",ethnicity:newIntern.ethnicity||"",discipline:newIntern.discipline||"",intern_type:newIntern.internType||"",credential:newIntern.credential||"",credential_body:newIntern.credentialBody||"",university:newIntern.university||"",license_goal:newIntern.licenseGoal||"",supervisor_role:newIntern.supervisorRole||"",start_date:newIntern.startDate||null,dob:newIntern.dob||null,status:"active",pro_bono:newIntern.proBono||false,group_ids:newIntern.groupIds||[],list_ids:newIntern.listIds||[],hours_completed:0,hours_total:newIntern.hoursTotal||0,individual_hours:0,group_hours:0,billing_rate:newIntern.billingRate||0,billing_schedule:newIntern.billingSchedule||"monthly"};const {data,error}=await supabase.from("interns").insert(row).select().single();if(data)setInterns(p=>[...p,{...newIntern,id:data.id,...data}]);else console.error(error);}}
