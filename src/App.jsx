    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    <div id="suptrack-content" style={{flex:1,padding:"38px 44px",maxWidth:980,overflowY:"auto"}}>
      ...
      {addInternOpen&&<AddInternModal T={t} groups={groups} lists={lists}
        onSave={async newIntern=>{const row={supervisor_id:session.user.id,name:newIntern.name,preferred_name:newIntern.preferredName||"",pronouns:newIntern.pronouns||"",discipline:newIntern.discipline||"",credential:newIntern.credential||"",license_goal:newIntern.licenseGoal||"",hours_completed:0,hours_total:newIntern.hoursTotal||0,billing_rate:newIntern.billingRate||0,billing_schedule:newIntern.billingSchedule||"monthly",status:"active",start_date:newIntern.startDate||null,pro_bono:newIntern.proBono||false};const {data,error}=await supabase.from("interns").insert(row).select().single();if(error){console.error("Intern insert error:",error);alert("Failed to save intern: "+error.message);return;}setInterns(p=>[...p,{...newIntern,id:data.id,...data}]);}}
